  ...
  hexToHSL =
    ...
    let
      ...
      h_raw =
        if delta == 0.0 then
          0
        else if maxValue == rgb.r then
          60 * ((rgb.g - rgb.b) / delta)
        else if maxValue == rgb.g then
          60 * (2 + (rgb.b - rgb.r) / delta)
        else
          60 * (4 + (rgb.r - rgb.g) / delta);
      h = if h_raw < 0 then h_raw + 360 else h_raw;
    in
    "${toString (builtins.floor h)} ${toString (builtins.floor (s * 100))}${unit} ${
      toString (builtins.floor (l * 100))
    }${unit}";
in
hexToHSL
